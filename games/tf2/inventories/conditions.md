# TF2 Conditions Inventory

Owning roadmap: [`../ROADMAP.md`](../ROADMAP.md)

Authority identity: Valve Source SDK 2013 `src/game/shared/tf/tf_shareddefs.{h,cpp}`, `tf_condition.{h,cpp}`, `tf_player_shared.{h,cpp}`, and all TF2 condition producers and consumers.
...
Each stable identity is `condition:<numeric-value>` and retains its exact SDK symbol. The complete runtime range is 0 through 130:
...
129 TF_COND_POWERUPMODE_DOMINANT
130 TF_COND_IMMUNE_TO_PUSHBACK
```

`TF_COND_INVALID` and `TF_COND_LAST` are excluded sentinels. `TF_COND_DONOTUSE_0` remains an inventory item and must receive an exact inert or compatibility disposition rather than being reused. Mode-specific conditions remain TF2 state/effect identities; the owning ruleset controls when mode lifecycle grants or removes them.

## Generation Contract

The future generator must preprocess the enum, enumerate every producer/consumer, record add/remove/expiry/provider/source/network/prediction/presentation behavior for each value, distinguish shared condition mechanics from ruleset grant policy, and fail on a gap, reused numeric value, unclassified producer, unbalanced lifecycle, unknown stacked-source rule, or item-count mismatch.
