...
export type Tf2Hud = Readonly<{
  ...
  weaponName: "Rocket Launcher" | "Original" | "Stickybomb Launcher" | "Scattergun" | "Pistol" | "Bat" | "Shotgun" | "Shovel" | "Minigun" | "Fists" | "Sniper Rifle" | "SMG" | "Kukri" | "Wrench" | "Flamethrower" | "Fire Axe" | "Bottle" | "Grenade Launcher" | "Construction PDA" | "Destruction PDA" | "Toolbox" | "Revolver" | "Knife" | "Sapper" | "Disguise Kit" | "Invisibility Watch" | "Syringe Gun" | "Medi Gun" | "Bonesaw" | null
...
export type Tf2AudioRequest = Readonly<{
  action?: Snapshot["audioEvents"][number]["action"]
  fadeSeconds?: number
  tick?: bigint
...
export function decodeModelPoseOutput(bytes: Uint8Array): readonly PosedModel[] {
  ...
  for (let count = view.getUint32(8, true); count > 0; count--) {
    ...
    const flexCount = u32()
    if (flexCount > primitives.length) throw new ProjectilePresentationError("BoundExceeded", "model flex primitive count")
    const flex = flexCount === 0 ? EMPTY_POSE_VALUES : Object.freeze(Array.from({ length: flexCount }, () => {
      const primitive = u32(), count = u32()
      const authored = primitives.find((value) => value.primitive === primitive)
      if (!authored || count > authored.vertexCount) throw new ProjectilePresentationError("MalformedFact", "model flex vertex count")
      const indices = new Uint32Array(count), positions = new Float32Array(count * 3), normals = new Float32Array(count * 3)
      for (let i = 0; i < count; i += 1) {
        const index = u32()
        if (index >= authored.vertexCount || i > 0 && index <= indices[i - 1]!) throw new ProjectilePresentationError("MalformedFact", "model flex vertex index")
        indices[i] = index
        for (let axis = 0; axis < 3; axis += 1) positions[i * 3 + axis] = f32()
        for (let axis = 0; axis < 3; axis += 1) normals[i * 3 + axis] = f32()
      }
    ...
    encode(tick: bigint, camera: Vector3, requests: readonly ProjectileParticleRequest[]) {
      ...
      for (const request of requests) {
        ...
        if (request.kind === "start") {
          ...
          length += 32
        } else if (request.kind === "set-flame-control-point") {
          const control=request.controlPoint
          if(!Number.isInteger(control.index)||control.index<1||control.index>30||!finite(control.position)||!quaternion(control.orientation)||!finite(control.velocity)||![control.radius,control.density,control.duration].every(value=>Number.isFinite(value)&&value>=0)||!uint32(control.ownerIdentity)){
            throw new ProjectilePresentationError("MalformedFact", "flame control-point request is invalid")
          }
          length += 60
      ...
      for (let index = 0; index < requests.length; index += 1) {
        ...
        if (request.kind === "start") {
          ...
          control.position.forEach((value,index)=>view.setFloat32(at+4+index*4,value,true))
          control.orientation.forEach((value,index)=>view.setFloat32(at+16+index*4,value,true))
          control.velocity.forEach((value,index)=>view.setFloat32(at+32+index*4,value,true))
          view.setFloat32(at+44,control.radius,true)
          view.setFloat32(at+48,control.density,true)
          view.setFloat32(at+52,control.duration,true)
          view.setUint32(at+56,control.ownerIdentity,true)
          at+=60
        }
      }
      previousTick = tick
      ...
      : snapshot.weapon === 40 ? "Shotgun"
        : snapshot.weapon === 41 ? "Pistol"
          : snapshot.weapon === 42 ? "Wrench"
            ...
            : (["", "Rocket Launcher", "Original", "Stickybomb Launcher", "Scattergun", "Pistol", "Bat", "Shotgun", "Shovel", "Minigun", "Shotgun", "Fists", "Sniper Rifle", "SMG", "Kukri", "Flamethrower", "Fire Axe", "Bottle", "Grenade Launcher", "Syringe Gun", "Medi Gun", "Bonesaw"] as const)[snapshot.weapon],
...
export function tf2Camera(snapshot: Snapshot, yawDegrees: number, pitchDegrees: number): Camera {
  return Object.freeze({
    ...
    yawDegrees: Math.fround(Math.fround(yawDegrees) + snapshot.viewAngleOffset[1]),
    pitchDegrees: Math.fround(Math.fround(pitchDegrees) + snapshot.viewAngleOffset[0]),
...
export type ProjectileParticleRequest =
  ...
  | Readonly<{
      kind: "set-flame-control-point"
      identity: string
      effectIdentity: string
      eventIdentity: string
      tick: bigint
      projectileIdentity: number
      controlPoint: FlameControlPoint
    }>
  | Readonly<{
      kind: "stop"
      identity: string
