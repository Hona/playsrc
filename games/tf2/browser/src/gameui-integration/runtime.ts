...
class Integration implements Tf2GameUiIntegration {
  ...
  #configureScrollers(mainMenu: VguiPanelId): void {
    ...
    for (const [scrollerName, childName] of [["MOTD_TextScroller", "MOTD_TextPanel"], ["Notifications_Scroller", "Notifications_Control"]] as const) {
      ...
      applyResourceTree(this.#runtime, scroller, source, cloneNode(source.root, [block]), this.#diagnostics, this.#resources.activeConditions, this.#resources.resolutionSuffixes)
    }
    void mainMenu
  }
  ...
  #applyOwned(logicalPath: string, owner: VguiPanelId, conditions: readonly string[] = []): void {
    ...
    if (self && snapshot.parent !== null) {
      ...
      applyResourceTree(this.#runtime, snapshot.parent, document, cloneNode(document.root, [shallow]), this.#diagnostics, [...this.#resources.activeConditions, ...conditions], this.#resources.resolutionSuffixes)
    }
    applyResourceTree(this.#runtime, owner, document, cloneNode(document.root, document.root.children.filter((block) => block !== self)), this.#diagnostics, [...this.#resources.activeConditions, ...conditions], this.#resources.resolutionSuffixes)
  }
  ...
  #apply(logicalPath: string, parent: VguiPanelId, conditions: readonly string[] = []): void {
    const document = this.#resources.document(logicalPath)
    applyResourceTree(this.#runtime, parent, document, document.root, this.#diagnostics, [...this.#resources.activeConditions, ...conditions], this.#resources.resolutionSuffixes)
  }
