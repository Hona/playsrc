...
function parseAudio(r: Reader): AudioArtifact {
  ...
  if (mixerGain < 0 || count < 1 || count > 32) throw new ArtifactError("audio document header")
  const documents = Object.freeze(Array.from({ length: count }, () => {
    const sourceSha256 = hex(r.take(32)), logicalPath = r.text()
    return Object.freeze({ sourceSha256, logicalPath, entries: Object.freeze(Array.from({ length: r.u32() }, () => soundNode(r))) })
  }))
  if (new Set(documents.map((document) => document.logicalPath)).size !== documents.length) throw new ArtifactError("audio document identity")
  return Object.freeze({ mixerSha256, mixerGain, documents })
}
