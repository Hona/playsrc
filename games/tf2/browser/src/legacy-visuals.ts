import type { LegacyVisualFrame, PixelVisibilityFeedback } from "@playsrc/rendering"
import type { VisibilityView } from "./protocol"

export type LegacyVisualView = VisibilityView & Readonly<{ viewportHeight:number;pixelVisibility:readonly PixelVisibilityFeedback[] }>

export function encodeLegacyVisualQuery(views:readonly LegacyVisualView[]):Uint8Array {
  if(views.length<1||views.length>2)throw new Error("Legacy visual view count")
  ...
  bytes.set(new TextEncoder().encode("PLVQ"));at=4;u32(1);u32(views.length)
  views.forEach((view,index)=>{
    ...
    u32(index);f32(view.presentationTimeSeconds);u32(view.viewportHeight);u32(view.pixelVisibility.length)
...
export function decodeLegacyVisualViews(bytes:Uint8Array):readonly LegacyVisualFrame[] {
  if(bytes.byteLength>4*1024*1024)throw new Error("Legacy visual output bound")
  const packet=new Reader(bytes);packet.header(3)
  const count=packet.u32();if(count<1||count>2)throw new Error("Legacy visual view count")
  const views=Array.from({length:count},()=>{
    const frame=new Reader(packet.take(packet.u32()));frame.header(2)
    const proxyCount=frame.u32(),quadCount=frame.u32()
    if(proxyCount>65536||quadCount>65536||proxyCount*88+quadCount*112!==frame.bytes.length-frame.at)throw new Error("Legacy visual record count")
    ...
    const quads=Object.freeze(Array.from({length:quadCount},()=>Object.freeze({source:frame.u32(),material:frame.u32(),frame:frame.u32(),hdrScale:frame.f32(),positions:frame.floats(12),color:frame.floats(4),uv:frame.floats(8)})))
    frame.end();return Object.freeze({proxies,quads})
  })
