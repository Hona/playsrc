...
function controlOwner(name: string): Tf2UiOwner | null {
  ...
  if (/^(?:C(?:AutoFittingLabel|AvatarImagePanel|CompetitiveAccessInfoPanel|CurrencyStatusPanel|CyclingAdContainerPanel|DashboardPartyMember|EmbeddedItemModelPanel|EventPlayListEntry|ExButton|ExImageButton|ExLabel|ExplanationPopup|IconPanel|ImagePanel|ItemModelPanel|MainMenuNotificationsControl|ModelPanel|PlayListEntry|PvPRankPanel|SteamFriendsListPanel|TeamMenu|TFArrowPanel|TFClassImage|TFClassTipsItemPanel|TFClassTipsPanel|TFFlagStatus|TFFooter|TFImagePanel|TFLogoPanel|TFPlayerModelPanel|TFTeamButton)|PanelListPanel)$/u.test(name)) return "tf2"
  return null
}
