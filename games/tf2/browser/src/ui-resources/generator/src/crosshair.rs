...
pub(crate) fn write(
    ...
    let weapons = vec![
        ...
        weapon(content, "scripts/tf_weapon_syringegun_medic.ctx", &[10])?,
        weapon(content, "scripts/tf_weapon_medigun.ctx", &[11])?,
        weapon(content, "scripts/tf_weapon_bonesaw.ctx", &[12])?,
    ];
