...
| Out-of-game Main Menu uses one full-screen proportional `CHudMainMenuOverride`, shows the configured resource background and selected character image, and preserves the configured descendants' design-space geometry; pause hides the character without creating a second root. | The integration enables proportional mode on the code-created owner before loading `mainmenuoverride.res`, recursively inherits that state into descendants, and reapplies aspect conditions plus resource geometry atomically on viewport changes. | Main Menu/pause lifecycle, proportional owner/descendant snapshots, 1,192×1,339, 1,280×720, 1,024×768, 2,560×1,080, 390×844 and 844×390 resize/DPR/restore captures pass. | Ready |
