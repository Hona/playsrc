...
| In-game GameUI activation expands the single configured dashboard with Find Game, Resume and Disconnect, hides Quit and the character background, retains configured shared account/settings controls, and never creates a second pause root. | Pause state owns the dashboard pause row plus configured shared play/account/settings catalogs; the resource integration applies one complete per-state visibility/capability map to existing code-created controls. | Main Menu/in-game/loading/failure/pause/disconnecting cross-product plus panel ancestry/count snapshots pass. | Ready |
