...
| Out-of-game Main Menu shows the configured resource background and selected character image; in-game GameUI activation expands the single configured dashboard with Find Game, Resume and Disconnect, hides Quit and the character image, retains configured shared account/settings controls, and never creates a second pause root. | The resource integration owns one complete per-state visibility/capability map over existing code-created controls and preserves the configured character-image disposition selected during initialization. | Main Menu/in-game/loading/failure/pause/disconnecting cross-product, character selection/visibility, panel ancestry/count snapshots, and headed 1,280×720 plus 390×844 captures pass. | Ready |
