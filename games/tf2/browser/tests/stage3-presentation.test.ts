...
test("decodes exact aligned PMPO vertex planes without copying and rejects non-finite or truncated geometry", () => {
  ...
  const planeStart = bytes.byteLength - 92
  const repeated = bytes.slice(12)
  const repeatedView = new DataView(repeated.buffer)
  repeatedView.setUint32(0, 10, true)
  const opacityOffset = Array.from({ length: 4 }, (_, index) => planeStart - 4 - index)
    .find(index => bytes[index] === 1 && bytes[index + 1] === 0 && bytes[index + 2] === 0 && bytes[index + 3] === 0)!
  repeated[opacityOffset - 12 + 1] = 1
  const reference = new Uint8Array(repeated.byteLength - 76)
  reference.set(repeated.subarray(0, planeStart - 12))
  reference.set(repeated.subarray(planeStart - 12 + 80), planeStart - 12 + 4)
  const shared = new Uint8Array(bytes.byteLength + reference.byteLength)
  shared.set(bytes)
  ...
  expect(second!.primitives[0]!.tangents).toBe(first!.primitives[0]!.tangents)
  const invalidReference = shared.slice()
  new DataView(invalidReference.buffer).setUint32(bytes.byteLength + planeStart - 12, 99, true)
  expect(() => decodeModelPoseOutput(invalidReference)).toThrow("model pose geometry reference")
