...
test("persistent authored view corrections survive snapshot decode and reject nonfinite angles", () => {
  ...
  view.setFloat32(bytes.byteLength - 132, -5, true)
  view.setFloat32(bytes.byteLength - 128, -150, true)
  expect(decodeSnapshot(bytes).viewAngleOffset).toEqual([-5, -150, 0])
  view.setFloat32(bytes.byteLength - 124, Number.NaN, true)
...
test("control point and overtime events admit every declared wave variant", () => {
  ...
  for (const [definition, waves] of [[85, 4], [86, 2], [89, 3], [90, 2], [91, 4], [98, 2], [119,10], [120,10], [121,1], [122,1], [158,1], [159,3]]) {
...
test("soundscape selection retains all eight positions and rejects malformed snapshot tails", () => {
  const bytes = snapshot(), view = new DataView(bytes), at = bytes.byteLength - 108
  expect(decodeSnapshot(bytes).soundscape).toMatchObject({ entity: 0, soundscape: -1, positionBits: 0 })
  view.setInt32(at, 9, true)
  view.setInt32(at + 4, 37, true)
  view.setUint32(at + 8, 0x81, true)
  for (let index = 0; index < 24; index++) view.setFloat32(at + 12 + index * 4, index - 12, true)
  const selected = decodeSnapshot(bytes).soundscape
  expect(selected).toMatchObject({ entity: 9, soundscape: 37, positionBits: 0x81 })
  expect(selected.positions).toHaveLength(8)
  expect(selected.positions[7]).toEqual([9, 10, 11])
  view.setInt32(at, 0, true)
  expect(decodeSnapshot(bytes).soundscape.positions).toEqual(selected.positions)
  for (const [offset, value] of [[0, -1], [4, -2], [8, 256]] as const) {
    const bad = bytes.slice(0), data = new DataView(bad)
    data.setInt32(at + offset, value, true)
    expect(() => decodeSnapshot(bad)).toThrow("soundscape selection is invalid")
  }
  view.setFloat32(at + 104, Infinity, true)
  expect(() => decodeSnapshot(bytes)).toThrow("soundscape selection is invalid")
  expect(() => decodeSnapshot(bytes.slice(0, -1))).toThrow()
})
...
test("critical draws preserve separate authority and predicted-presentation records", () => {
  const source = new Uint8Array(snapshot()), at = 985
...
test("player weapon counters retain full native heads and reject invalid revenge counts", () => {
  const bytes = snapshot(), view = new DataView(bytes)
  view.setInt32(bytes.byteLength - 120, 800, true)
  view.setInt32(bytes.byteLength - 116, 35, true)
  expect(decodeSnapshot(bytes)).toMatchObject({ decapitations: 800, revengeCrits: 35 })
  view.setInt32(bytes.byteLength - 116, 36, true)
...
test("native crosshair scale follows counters and rejects non-finite values", () => {
  const bytes = snapshot(), view = new DataView(bytes)
  view.setFloat32(bytes.byteLength - 112, 1.625, true)
  expect(decodeSnapshot(bytes).weaponCrosshairScale).toBe(1.625)
  view.setFloat32(bytes.byteLength - 112, NaN, true)
...
test("projectile wave cycles do not overwrite main objective or configured sound masks", () => {
  ...
  bytes[random + 293] = 15
  view.setUint16(random + 294, 0xffff, true)
  bytes.set([255, 255, 7, 0], random + 360)
  expect(decodeSnapshot(bytes).randomState).toMatchObject({ overtimeAvailable: 15, controlPointAvailable: 0xffff,
    projectileUnlockAvailable: [7, 7, 7, 7, 15, 7] })
  bytes[random + 363] = 1
  expect(() => decodeSnapshot(bytes)).toThrow("TF2 sound selection state is invalid")
})

test("payload alert masks preserve ordinals eight and nine and reject old random-state framing", () => {
  const bytes = snapshot(), view = new DataView(bytes), random = 617
  view.setUint16(random+364,0x380,true)
  expect(decodeSnapshot(bytes).randomState.payloadWarningAvailable).toEqual([0x380,0x3ff])
  view.setUint16(random+366,0x400,true)
  expect(()=>decodeSnapshot(bytes)).toThrow("TF2 sound selection state is invalid")
  view.setUint16(random+366,0x3ff,true)
  view.setUint32(random+4,3,true)
  expect(()=>decodeSnapshot(bytes)).toThrow("TF2 random state identity is invalid")
})
...
function rosterSnapshot(tick: bigint, roster = 31, brushes = 512): Uint8Array {
  ...
  const objective = original.length - 292, brushHeader = objective - 64
  ...
  bytes.set(original.subarray(objective + 84, original.length - 132), objective + insert + botBytes + 84 + scoreboardBytes)
  bytes.set(original.subarray(original.length - 132), bytes.length - 132)
  const view = new DataView(bytes.buffer)
  ...
  for (let i = 0; i < roster; i++) {
    ...
    view.setFloat32(bytes.length - 132 - roster * 28 + i * 28 + 24, 88, true)
  }
  return bytes
}
...
describe("TF2 canonical gameplay command and snapshot contract", () => {
  ...
  test("accepts every authored KOTH and capture announcer wave ordinal, rejecting the first out-of-range wave", () => {
    for (const [definition, waveCount] of [[85, 4], [86, 2], [89, 3], [90, 2], [91, 4], [98, 2], [103, 1], [104, 1], [105, 1], [106, 1], [107, 1], [108, 1]] as const) {
      const source = rosterSnapshot(1n, 0, 0), at = 985
  ...
  test("Worker snapshot deltas and coalescing retain ordered sound patch starts, destruction and re-press", () => {
    const frame = (tick: bigint, events: readonly (readonly [number, number, number])[]) => {
      const base = rosterSnapshot(tick, 0, 0), at = 985
    ...
    pitched[985 + 11] = 115; pitched[985 + 15] = 4
    new DataView(pitched.buffer).setFloat32(985 + 48, 92, true)
    ...
    for (const invalid of [5, 255]) {
      const bad = first.slice(); bad[985 + 15] = invalid
      expect(() => decodeSnapshot(bad.buffer)).toThrow(Tf2CodecError)
    }
    const invalidDuration = first.slice()
    new DataView(invalidDuration.buffer).setFloat32(985 + 48, NaN, true)
  ...
  test("decodes every canonical class identity and a genuinely unarmed class snapshot", () => {
    for (let identity = 1; identity <= 9; identity += 1) {
      ...
      if (identity === 8) {
        ...
        const view = new DataView(bytes.buffer), cloak = base.length - 136
  ...
  test("encodes bounded bot commands and decodes ordered player lifecycle snapshots", () => {
    ...
    view.setFloat32(bytes.byteLength - 136, 88, true)
    view.setFloat32(bytes.byteLength - 112, 1, true)
  ...
  test("randomized lossless full/delta sequence parity retains all event ticks and large rosters", () => {
    ...
    for (let frame = 0; frame < 100; frame++) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const view = new DataView(state.buffer), brushHeader = 1049
  ...
  test("immutable sections, signed zero, caller-owned collision bytes and shutdown do not alias baselines", () => {
    ...
    thirdView.setBigUint64(8, 9n, true); thirdView.setFloat32(1049 + 52 + 40, -0, true)
  ...
  test("malformed, stale, NaN, cross-section and truncated responses roll back the entire decode", () => {
    ...
    for (const offset of [20, 204, 1049 + 52 + 40]) {
  ...
  test("identical objective events are retained once for each exact tick, not deduplicated with state", () => {
    const withObjective = (tick: bigint) => {
      const base = rosterSnapshot(tick, 31, 1), at = 1125 + 128 + 31 * 128
