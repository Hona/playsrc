...
- Project Soldier/Demoman class speed, condition/item speed and jump factors, air-control cap, 1.2-times bunnyhop cap, backward clamp, TF2 hull/view values, jump eligibility/impulse, crouch eligibility, surface factors, ruleset noclip permission, eyes-before-raised-waist water sampling, airborne/submerged duck suppression, class-speed swim intent, surface-transition gravity, and 30-unit/300-unit water exits through one policy consumed by generic Movement.
- Preserve the separately retained Source in-water flag, stock Soldier/Demoman self-blast damage and force ordering, normalized water/slime type, standing versus crouched blast volume, and accepted next-tick dry/feet/waist/eyes/submerged velocities without inventing underwater force attenuation.
...
- Interpret selected entity integer, float, and boolean fields through Source KeyValues typed accessors while retaining raw entity strings and caller-owned defaults and range checks.
- Declare exact downloaded and installed TF2 map targets; run `pl_upward` through the existing Jump/noclip direct-map session without Payload rules, scoring, HUD, timers, carts, checkpoints, overtime, or wins.
