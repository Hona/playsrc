...
| Local and deployed origins implement the same routes, bytes, status, validators, cache, range, integrity, cancellation, and error behavior for the same Asset Store snapshot. | No local asset process or deployed origin binding exists. | **Dual-origin conformance:** serve one fixed store through loopback and isolated deployed origin; compare complete response transcripts byte-for-byte except declared Date/server transport fields. | Blocked |
...
- **Deployed origin:** the Infrastructure roadmap is Draft and defines no accepted origin compute, remote Asset Store binding, CDN behavior, cache-key policy, no-transform enforcement, TLS, CORS origin binding, or isolated evidence environment for `assets.playsrc.online`.
