...
- Run one direct-BSP TF2 worker with Rust-owned Source Simulation pacing and browser metrics clock, Three.js/WebGPU world plus inline brush models, selected-teamspawn camera initialization, Source-signed pointer look, physical left/right Shift crouch, separate VGUI mount, exact blocker ledger, and generation-safe map replacement.
...
- Consume PSSN v7 random/audio/Collision/pusher/Entity brush facts, 123 brush descriptors, PMRQ/PMPO v3 hand-plus-item draw plans, Particle v3 materials/sheets/trails, Source Audio, and configured macOS SourceScheme fonts. IVP remains explicit: sticky fire is rejected before spawn. Native glyph-raster and aligned visual captures remain blockers.

Run the checked end-to-end browser procedure with `bun run verify:browser jump_beef`. It starts the same checked development owner as `bun run dev jump_beef`, drives cold and warm headed-browser runs, retains a fixed 1,280×720 canvas capture plus exact camera/decal/model/depth metadata, rejects missing spawn-room surfaces or reversed pointer direction, and verifies one-interrupt process shutdown.
