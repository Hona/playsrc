...
- Accept one bounded configuration containing exact `jump_beef` and `pl_upward` product identities plus one default. Console switches either exact target through its catalog graph, BSP, loading descriptors, and dependency closure while preserving one Worker/WASM authority; aliases and unknown targets fail before fetch or state mutation. Preserve Jump and noclip authority, and attach no Payload meaning to parsed stock-map entities.
