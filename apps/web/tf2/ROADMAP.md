...
| `TF2-WEB-008` — Preact mounts one product shell and one framework-independent VGUI root; TF2-owned immutable HUD bindings update VGUI and typed VGUI commands return through the application without either owner adopting the other's DOM. | No complete VGUI package, TF2 HUD binding inventory, or application mount exists. | **DOM ownership and HUD seam audit:** instrument shell, VGUI, and canvas subtrees under fixed gameplay/replay states; compare node owners, bindings, typed commands, focus/accessibility state, and deterministic destruction. | Blocked |
...
| `TF2-WEB-013` — The application console adapter accepts exact VGUI submissions, completes from owner snapshots, and implements `map <catalog-map-name>` plus the playsrc-only `map https://<allowed-origin>/<path>/<map-name>.bsp` operation without moving command, convar, Content, map, or Simulation authority into VGUI. | VGUI now exposes the bounded typed submission, completion, cancellation, output, visibility, and lifecycle seam. No application command/convar registry adapter, external acquisition adapter, direct-load integration, or accepted origin policy exists. | **Console-to-map adapter transcript:** fixed empty, catalog boundary, unknown, exact URL, credentials, fragment, port/origin, redirect, CORS, absent/incorrect/limit-plus-one length, timeout, cancellation, malformed/corrupt BSP, missing dependency, provider failure, stale generation, success, reload, and publication schedules compare VGUI requests/output, owner calls, progress, hashes, route, prior-state preservation, observations, and zero VGUI execution. | Blocked |
...
- **VGUI and HUD bindings:** generic VGUI ownership is resolved, but no accepted TF2 HUD binding inventory or application mount exists. `TF2-WEB-008` cannot pass integration review. Checked KeyValues, VGUI, TF2, root, and shared-web roadmaps.
- **Console map adapter:** no application command/convar adapter, accepted external-origin policy, bounded HTTPS BSP acquisition, `/jump/external` coordinator, or direct-source replacement integration exists. `TF2-WEB-013` remains Blocked; VGUI cannot supply this application behavior.
