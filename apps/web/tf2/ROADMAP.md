...
| `TF2-WEB-003` — `/` presents the two catalog entries; `/jump/<map>` selects one exact catalog map source/course identity; `/replay/upload` selects no map source; canonical navigation never infers an experience from content or file names. | No TF2 router or route-state model exists. | **Route/selection decision table:** direct-load, link, reload, history, malformed map, absent entry, and uploaded filename cases compare canonical URL, selected map source/course, and failure exactly. | Not started |
