...
| `TF2-WEB-013` — The application console adapter accepts exact VGUI submissions, completes from owner snapshots, and implements `map <catalog-map-name>` plus the playsrc-only `map https://<allowed-origin>/<path>/<map-name>.bsp` operation without moving command, convar, Content, map, or Simulation authority into VGUI. | The generic console presentation seam is not implemented, and no application command registry, external acquisition adapter, direct-load integration, or accepted origin policy exists. | **Console-to-map adapter transcript:** fixed empty, catalog boundary, unknown, exact URL, credentials, fragment, port/origin, redirect, CORS, absent/incorrect/limit-plus-one length, timeout, cancellation, malformed/corrupt BSP, missing dependency, provider failure, stale generation, success, reload, and publication schedules compare VGUI requests/output, owner calls, progress, hashes, route, prior-state preservation, observations, and zero VGUI execution. | Blocked |
