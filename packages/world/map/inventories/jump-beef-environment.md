...
The surface material's declared environment path selects cubemap sample 0 under both profiles. Reflection and refraction remain built-in framebuffer requests; the normal map and beneath material remain exact external dependencies. The beneath material reports `aboveWater=false`; the surface material reports `aboveWater=true`.

## Marks

The entity graph contains 39 ordered `infodecal` records. Exact receiver evidence classifies 34 projected, four ineligible, and one missing material, producing 63 bounded fragments and 252 vertices. Every receiver polygon is clipped against the decal unit square in Source plane-basis order, and every clipped position is moved 0.1 Source units along its stored BSP plane normal. The canonical source/status/material/model/face/position/UV/index stream defined in [`jump-beef-surfaces.md`](jump-beef-surfaces.md) has SHA-256 `e413f890e38e010f926fa10ad65a78c1ce9f85f68583931112059240439a8491`. Standard overlay and water-overlay lumps are empty. Empty compiled mark lumps are intentionally inert; an unknown nonempty version or malformed whole record is not.
