...
- Retain canonical source-ordered static-prop v6/v10 model/occurrence tables with normalized model dependencies, exact transforms, nonempty non-solid BSP leaf memberships, solidity, inert padding, skins, fades, DX levels, flags, lightmap resolution, and explicit optional lighting origins; reject missing or solid membership leaves before publishing a map.
- Reject prepared Entity authorities whose retained bytes differ from the selected BSP entity lump. Produce source-ordered typed environment controllers independently of material closure; preserve zero-controller 3D-sky leaf visibility and exact authored sky-controller origin, scale, fog, and BSP area without inventing a controller.
...
Complete Map parity evidence runs through `bun packages/world/map/scripts/verify-parity.ts` and requires the repository-root local configuration, exact cached `jump_beef` BSP/source bundle, and exact installed `pl_upward` BSP. The dual-map vector verifies controller-independent sky flags, source-ordered controller state, 558 displacement collision grids, 1,244 static props, 2,480 indexed VHV objects, and separate main/3D-sky area membership. The narrower brush presentation workflow remains `bun packages/world/map/scripts/verify-brush-model-presentation.ts`.
