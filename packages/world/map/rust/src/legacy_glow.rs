...
impl LightGlow {
    ...
    pub fn quad(&self, view: &View, position: [f32; 3], direction: [f32; 3], color: [u8; 3], visibility: f32) -> Option<Quad> {
        ...
        let (to_glow, _) = normalize(sub(position, view.origin));
        let center = add(view.origin, scale(to_glow, 100.0));
        let (right, _) = normalize(cross(to_glow, [0.0, 0.0, 1.0]));
        let (up, _) = normalize(cross(right, to_glow));
        let right = scale(right, self.horizontal_size as f32);
        let up = scale(up, self.vertical_size as f32);
        // CMeshBuilder::Color4f packs through Source's FastFToC before upload.
...
mod tests {
    use super::*;
    fn view() -> View { View { origin: [0.0;3], forward: [1.0,0.0,0.0], right: [0.0,-1.0,0.0], up: [0.0,0.0,1.0], world_to_clip: [[0.0;4];4], height: 720 } }
