...
| Item count | 42 Candidate: 25 Handled, 16 Unsupported, 1 Missing; 0 Accepted |
...
| VIS-002 | Node topology | Validate stable node identity, plane reference, two child references, bounds, and area summary. | Handled |
| VIS-003 | Leaf topology | Validate stable leaf identity, bounds, contents, cluster, area, flags, and map memberships. | Handled |
| VIS-004 | Cluster universe | Derive the finite cluster set and validate every leaf and visibility-row reference. | Handled |
| VIS-005 | World head node | Validate and retain the world model's starting node or encoded leaf. | Handled |
| VIS-006 | Point-to-leaf query | Traverse the world tree with exact negative/zero/positive plane-side behavior. | Handled |
| VIS-007 | Point-to-cluster query | Return the resolved leaf's cluster, including valid cluster `-1`. | Handled |
| VIS-008 | AABB-to-leaves query | Enumerate every intersected leaf once under declared result limits. | Handled |
| VIS-009 | Head-node visibility | Decide whether any descendant leaf has a cluster bit set. | Unsupported |
| VIS-010 | PVS offsets | Validate one PVS compressed-row offset per cluster. | Handled |
| VIS-011 | PVS decompression | Expand bounded literal and zero-run bytes to one exact cluster row. | Handled |
| VIS-012 | PVS queries | Answer cluster, leaf, point, head-node, and box potential visibility. | Unsupported |
| VIS-013 | PAS offsets | Validate one PAS compressed-row offset per cluster. | Handled |
| VIS-014 | PAS decompression | Expand bounded PAS bytes with the same row and padding invariants as PVS. | Handled |
| VIS-015 | PAS queries | Return potentially audible cluster, leaf, point, and box results without recipient policy. | Unsupported |
| VIS-016 | Multi-origin union | Union 1 through 32 visibility origins after exact point-to-cluster lookup. | Handled |
| VIS-017 | Empty visibility lump | Treat a valid empty visibility lump as all PVS and PAS clusters visible. | Handled |
...
| VIS-019 | Outside-world view | Return all visibility-gated candidates when a finite camera origin resolves to cluster `-1`. | Handled |
| VIS-020 | Area records | Validate area identity and each contiguous directed-portal range. | Handled |
| VIS-021 | Directed area portals | Validate portal key, destination area, plane, and clip-vertex range. | Handled |
| VIS-022 | Portal open state | Apply ordered key-based open/close updates atomically at one revision. | Handled |
| VIS-023 | Area connectivity | Return deterministic transitive connectivity under current portal state. | Handled |
...
| VIS-031 | Sky classification | Return `3D sky`, `2D sky`, or `not visible` with declared multi-origin precedence. | Handled |
| VIS-032 | World-surface membership | Retain map-owned surface-to-leaf memberships and emit visible surface IDs once. | Handled |
| VIS-033 | Static-prop membership | Retain compiled prop leaf lists and emit visible prop IDs once. | Handled |
| VIS-034 | Dynamic-object membership | Link current finite AABBs to leaves and invalidate membership after every bounds revision. | Handled |
| VIS-035 | Detail-prop membership | Retain compiled per-leaf ranges and emit candidates without placement, fade, or draw behavior. | Handled |
| VIS-036 | Ordered world list | Emit front-to-back leaves and first-encounter deduplicated world and object candidates. | Handled |
...
| VIS-040 | View cache and invalidation | Key complete results by exact view and state revision and invalidate changed dependencies. | Handled |
