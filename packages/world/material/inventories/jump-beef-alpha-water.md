...
Source identity: configured TF2 content build `24245096`, `jump_beef` resource root. Its complete configured graph contains 1,169 entries, 379 VMTs, and 388 VTFs; the gameplay-role closure contains 309 entries, 109 VMTs, and 127 VTFs. The selected gameplay VMT set contains 55 model materials, 12 projectile-particle materials, 13 present mark materials, the world fence and glass families, and both effective Water roots.
...
The surface is above water with reflection `0.25`, refraction `0.32`, fog `{51 43 13}` from `1` through `400`, and a linked beneath material. The beneath material is below water with no environment/reflection binding, refraction `0.5`, tint `[0.95 1.0 0.97]`, fog `{92 100 80}` from `-350` through `1050`, blur enabled, and overlay `effects/water_warp_2fort`. Neither material authors `$translucent`; both Water shader draws are opaque and depth-writing even though their refraction targets make the view's water-render policy nonopaque. WaterLOD supplies `1000..2000`. No configured Water VMT defines a base texture or flow map; neither role is inferred from another texture.
