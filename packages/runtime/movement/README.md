...
- Consume game-supplied hulls, views, speed/air/bunnyhop/backward/jump values, eligibility, step strategy, water-sampling order/height, current policy, walk-entry sampling, swim-jump/pitched-wish policy, underwater duck restrictions, ledge approach policy, and accepted mode-transition dispositions. Immutable tracer extensions supply point contents, surface climbability, support/conveyor velocity, mover displacement/filtering, observer targets, and the monotonic clock required only by incremental stuck recovery.
- Normalize stored fluid type to Source water/slime bits while retaining raw point-content query facts; preserve generic midpoint-before-eyes/current behavior and game-selected eyes-before-raised-midpoint/no-current swimming through one fixed-tick movement authority.
