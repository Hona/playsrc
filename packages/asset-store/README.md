...
- Fetch browser immutable objects through exact hash URLs with HTTP-cache reuse and independent byte verification; retain verified derived objects as Blob-backed records in the version-3 IndexedDB cache under caller-owned exact derived keys, with every request and transaction terminating within 30 seconds. A successful complete read advances eviction recency only after byte verification; same-millisecond accesses receive a deterministic strict order.
