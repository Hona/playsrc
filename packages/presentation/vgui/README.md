...
- Preserve Label-descendant alignment/insets, authored `CExLabel` foreground and shadow scheme colors, exact normal/armed/depressed/selected/focused/disabled colors and borders, slider track/thumb state, PropertySheet tabs, SectionedList column flags, and one workspace-positioned ComboBox Menu popup with separate selected, armed and disabled rows.
