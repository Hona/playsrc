...
- All 13 present decal materials supply source-alpha/one-minus-source-alpha blending, back-face culling, depth test with no depth write, nearer-or-equal comparison, and categorical decal offset. PENV v2 carries 73 collision-selected fragments; the fixed spawn admits exactly 13 by world/brush receiver visibility and current transforms.
...
- PTF2 v8 contains 55 typed model materials, exactly three EyeRefract states, 52 VertexLitGeneric states, 71 unique authored model textures, 2,761 decoded model subresources, plus complete selected alpha and Water texture chains. Rendering validates topology, dimensions, scalar/color interpretation, wrap/min/mag state, and anisotropy; it never generates model, alpha, or Water mips. Five opaque world materials still expose mip-zero-only diagnostics.
...
- PENV v2 carries complete mark receiver/revision/lifetime/pass state, Water surface/bottom bindings, controller values, leaf distances, and decoded selected alpha/Water/sky/cubemap subresources. PMST v2 carries typed alpha ownership and discard.
- PVIS v2 carries origin leaf/leaves/areas/sky state plus evaluated Water frame/transform/WaterLOD, above/below/intersection classification, reflection/refraction/main/intersection plans, clips/fog, and one surface set per view. Rendering executes bounded Water targets and restores camera, target, clipping, background, entity, and visibility state.
- Fresh native/WASM verification passes 317 inputs, LDR 42,082,929 bytes at `56153098…156`, HDR 78,256,304 bytes at `0f33e861…84a5`, 475,511,805 presentation bytes, fixed 91-surface spawn PVS, frame-30 above/below Water plans, near-plane intersection, stock/Original launch, held cadence, and configured brush solidity.
