...
- Require one unique exact StudioModel matrix for every map occurrence; reject posed material/template mismatches; update posed positions, normals, and tangent handedness; and draw viewmodels through a separate horizontal-4:3-FOV projection, world far plane, WebGPU viewport depth range `[0,0.1]`, restored world range `[0,1]`, and post-world pass.
