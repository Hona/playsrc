...
- Decode schema-5 displacement metadata, geometry and vertex alpha; retain source-ID draw evidence; consume adapted ordinary-world winding, Source normals and natural PVS/frustum admission; blend WorldVertexTransition/detail state; and preserve back-cull/depth/lightmap behavior.
...
- Consume PENV v3 profile-selected sky encoding, sky/cubemap inputs, and authored Water texture chains plus PVIS v4 PVS/frustum and above/below/reflection/refraction/main/intersection plans. Exact modules build Source-axis 2D-sky faces/seams, RGBS decode taps, authored six-face cubemap mips/sampling, cheap/expensive Water and fog probes, and Source-ordered Water views. The configured HDR sky supplies one `$hdrcompressedTexture` RGBS face per semantic side. Water tangent/projected-depth/overlay material inputs remain missing, so the diagnostic adapter never substitutes a default cubemap or approximate Water shader.
- Consume PTF2 v12/PVHA v2 static-prop records as source-backed color attributes over shared model geometry/material/texture resources; apply exact skin/root LOD/transforms, distance/screen fades, PVS leaf admission, unchanged PC BGRA vertex lighting and four runtime ambient/direct-light records.
- Execute the configured authored 3D sky as an area-filtered scaled view with independent visibility origin, fog, world/displacement/static-prop draws, depth reset and complete main-state restoration. The six-face 2D sky remains a separate visibility path.
...
- Retain independent main/3D-sky face-posting indices and render bundles, preserve original opaque triangle order and reverse translucent face order, upload only changed index ranges, and resolve static occurrences from indexed visible leaves with exact authored ownership, distance/screen fade, immutable bounds, and direct four-source runtime-light diagnostics.
- Retain dynamic brush/model/viewmodel instances and authored geometry/material resources by occurrence identity; write binary32-identical Particle sprite/trail quads and sheet/color data without per-particle temporary vectors, sliced arrays, or recreated consecutive batch descriptors.
- Execute the authored 2D sky before opaque world bundle commands and flush those commands before ordinary opaque/translucent rocket trail, impact flash, debris, and smoke draws; preserve exact SpriteCard blend/alpha/depth-test/no-depth-write state, real intervening-wall occlusion, stable source order, independent 3D-sky/Water passes, and post-world viewmodel depth isolation.
- Draw projected marks from supplied model-local fragments and authored alpha/mips with exact world/brush receiver-face joins, compiled overlay order, current receiver transforms and visibility, Source reciprocal decal slope, depth24plus-scaled constant bias, and preserved opaque-wall depth; present textures never become diagnostic quads.
