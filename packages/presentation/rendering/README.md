...
- Consume PENV v3 profile-selected sky encoding, sky/cubemap inputs, and authored Water texture chains plus PVIS v2 above/below/reflection/refraction/main/intersection plans. Exact modules build Source-axis 2D-sky faces/seams, RGBS decode taps, authored six-face cubemap mips/sampling, cheap/expensive Water and fog probes, and Source-ordered Water views. The configured HDR sky supplies one `$hdrcompressedTexture` RGBS face per semantic side. Water tangent/projected-depth/overlay material inputs remain missing, so the diagnostic adapter never substitutes a default cubemap or approximate Water shader.
