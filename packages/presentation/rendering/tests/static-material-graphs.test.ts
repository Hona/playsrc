...
test("same-plane static templates reuse compiler state without merging another texture or draw fade", () => {
  ...
  for(let index=0;index<3;index++)owner.admit(`opaque${index}`,`template${index}`,geometry,bases[index%2],state,false,false,THREE.FrontSide)
  for(let index=0;index<3;index++)owner.admit(`fading${index}`,`fade-template${index}`,geometry,bases[0],state,false,true,THREE.FrontSide)
  const result=owner.finish()
  expect(result.draws).toBe(6)
  expect(result.dedicatedCompilerStates).toBe(6)
  expect(result.sharedCompilerStates).toBe(3)
