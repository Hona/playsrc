...
test("bounds frame opportunities, coalesces in-flight work, records suspension, and surfaces failure", async () => {
  ...
  const records = pacing.records()
  expect(records).toHaveLength(4)
  expect(pacing.records()).toBe(records)
  expect(records.map((record) => record.disposition)).toEqual(["accepted", "suspended", "accepted", "stopped"])
  await pacing.offer(80, async () => ({ submitMilliseconds: 81, presentMilliseconds: 82 }))
  expect(pacing.records()).not.toBe(records)
  expect(pacing.records().at(-1)?.disposition).toBe("stopped")
})
