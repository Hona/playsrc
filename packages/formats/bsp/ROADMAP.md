...
|---|---|---|---|
| A caller-selected accepted profile validates `VBSP`, container version, the fixed 64-entry directory, and the signed map revision without branch guessing. | No BSP implementation exists; the profile inventory is Candidate and its parent format inventory is Not accepted. | Fixed v19, v20, and v21 headers: compare every decoded header field and raw directory byte with an independent little-endian field extractor; require exact rejection offsets for bad identifier, profile/version mismatch, and truncated headers. | Blocked |
| Every standard lump descriptor is range-checked while preserving file order independence, unaligned offsets, partial overlaps, exact duplicate ranges, zero-length metadata, and the raw fourth field. | No BSP implementation exists. | Synthetic directories covering each boundary at 0, 1, 1,035, 1,036, and end-of-file; compare retained ranges and overlap relations exactly, and require deterministic Malformed results for negative, overflowing, and truncated ranges. | Not started |
| Source-profile compressed standard lumps decode the 17-byte Source LZMA envelope only when the directory declares compression, and retain both encoded and decoded bytes. | No BSP implementation exists. | Fixed uncompressed and compressed lumps: compare identifier, sizes, properties, encoded bytes, decoded bytes, and byte-exact re-emission; reject mismatched sizes, short headers, short payloads, decode failure, and budget excess. | Not started |
| All 64 standard slots produce the typed or explicitly opaque representations in [`inventories/lumps.md`](inventories/lumps.md), including entity bytes, geometry, displacements, overlays, lighting, visibility bytes, leaf data, physics framing, and branch-specific slots. | No BSP implementation exists; exact declared-build use and the internal layout of legacy CS:GO slots 49 and 62 are not established. | For every accepted inventory profile, compare record counts, raw record bytes, decoded scalar bit patterns, variable-table boundaries, tails, and classifications against an independent extractor over fixed declared-build maps and isolated structural vectors. | Blocked |
| Counted, indexed, and variable records validate only structural arithmetic owned by BSP and retain semantic references for their downstream owner. | No BSP implementation exists. | Boundary vectors for visibility tables, occlusion sections, displacement sample streams, physics block framing, string offsets, overlays, ambient indexes, and branch prop-hull tables; require exact range results without constructing map, collision, visibility, physics, or rendering state. | Not started |
...
| Lump 40 parses bounded ZIP32 PAK data with methods 0 and 14 while retaining archive order, raw names, duplicate names, comments, extras, local/central metadata, encoded payloads, and CRC results. | No BSP implementation exists. | ZIP vectors for empty archives, stored and ZIP-LZMA entries, local/central disagreement, duplicate names, comments, extras, CRC mismatch, unsupported methods, encrypted flags, truncated records, count limits, decoded-byte limits, and expansion limits; compare decoded bytes and exact archive retention. | Not started |
...
| Every parse and decode operation checks caller-supplied allocation and expansion limits before allocation and classifies every encountered identity or failure without silent fallback. | No BSP implementation exists. | Instrumented fixed-input runs at each limit minus one, equal to the limit, and plus one; retain allocation counters and require no allocation above the declared budget, no panic, one classification per value, and identical repeated results. | Not started |
| Successful parsing is lossless and independently consumable by content, map, material, entity, collision, visibility, physics, rendering, tools, and the selected game without creating a second parsing authority. | No BSP implementation or consumer integration exists. | Byte-exact parse/re-emit comparisons for all fixed maps; integration records proving each named consumer reads the current typed interface and that repository search finds no duplicate BSP reader, compatibility path, fallback, or stale generated artifact. | Not started |
