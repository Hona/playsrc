...
| All 64 standard slots produce the typed or explicitly opaque representations in [`inventories/lumps.md`](inventories/lumps.md), including entity bytes, geometry, displacements, overlays, lighting, visibility bytes, leaf data, physics framing, and branch-specific slots. | Source-2013 v20 records consumed by the declared `jump_beef` build are typed: entities, planes, texture data, vertices, visibility framing, nodes, texture info, faces, lighting, leaves, edges, surface edges, models, leaf-face and leaf-brush indexes, brushes, brush sides, vertex normals and indexes, compiled primitives/vertices/indices, PAK, cubemaps, texture string data and offsets, HDR lighting, and HDR faces. Every non-empty unimplemented slot remains exact opaque bytes classified `Unknown`; an implemented slot with an unhandled version is `Unsupported`. Complete accepted-profile inventory coverage and legacy CS:GO slots 49 and 62 remain blocked. | Fixed vectors compare float bit patterns, padding, record boundaries, counts, budgets, unknown slots, and unsupported versions. The exact `jump_beef` input parses 27 handled non-empty slots and retains 18 non-empty unknown slots; typed counts include 3,793 LDR faces, 3,793 HDR faces, 126 compiled primitives, 1,659 primitive indices, 1,899 leaves, 476 brushes, 2,856 brush sides, and three cubemaps. | Partial |
| Counted, indexed, and variable records validate only structural arithmetic owned by BSP and retain semantic references for their downstream owner. | Fixed records and the visibility cluster count, ordered PVS/PAS offset pairs, compressed-byte tail, range, and record budget are implemented. Occlusion sections, displacement sample streams, physics blocks, game lumps, and remaining variable records are not implemented. | Visibility vectors cover exact framing, truncation, negative count, and count budget. The declared map retains 284 cluster offset pairs and its compressed tail without PVS expansion. | Partial |
