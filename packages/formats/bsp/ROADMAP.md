...
| The game-lump parent validates its count and directory, derives compressed extents without overflow, preserves entry order and duplicates, and exposes unknown entries losslessly. | The active static-prop checkpoint adds bounded ordered directory decoding and exact Source-LZMA child extents for the selected `sprp` member. General retained unknown/duplicate payload output remains absent. | Synthetic unsorted/compressed/final-child vectors plus exact configured directory evidence. | Partial |
| The four required game-lump identities and ten accepted layout profiles in [`inventories/game-lumps.md`](inventories/game-lumps.md) decode exact dictionaries, leaf lists, records, counts, padding, flags, and tails. | The active checkpoint implements `Source2013V20` `sprp` v10/72-byte records and the SDK-defined v6/64-byte records used by the declared `jump_beef` object. V6 promotes its byte flags, applies the SDK's no-per-texel-lighting bit, and supplies zero lightmap resolution; both profiles retain raw source indexes, flag-selected optional lighting origin, and strict framing. Other accepted profiles remain absent. | Synthetic v6/v10 field and boundary vectors plus exact configured zero-occurrence v6 and 234-model/2,756-leaf/1,244-occurrence v10 inventories. | Partial |
