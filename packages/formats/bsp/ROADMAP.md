...
| The four required game-lump identities and ten accepted layout profiles in [`inventories/game-lumps.md`](inventories/game-lumps.md) decode exact dictionaries, leaf lists, records, counts, padding, flags, and tails. | The active checkpoint implements the explicit `Source2013V20` + `sprp` v10 + 72-byte profile with raw source indexes, all fields, flag-selected optional lighting origin, and strict record framing. Other accepted profiles remain absent. | Synthetic v10 boundaries and exact configured 234-model/2,756-leaf/1,244-occurrence inventory. | Partial |
