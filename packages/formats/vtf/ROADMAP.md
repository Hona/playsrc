...
The active migration checkpoint implements the explicit `source-2013-pc` dialect for VTF 7.0 through 7.5; exact sequential/resource-directory image framing; 2D, animated, volume, and Source-2013 cubemap descriptors; selected low/high subresources; bounded opaque custom resources; and native U8 planes for BGR888, BC1/DXT1, one-bit-alpha BC1, and BC3/DXT5 required by the 27 `jump_beef` texture dependencies. Other image-format codes remain explicit `Unsupported` or `Unknown`.
...
| Animated texture frames remain independently selectable and retain ascending on-disk order; VTF supplies no frame duration. | Every frame has an independent identity and no timing field exists. | Two-frame synthetic and 60-frame exact water-normal data pass. | Ready |
| A cubemap exposes the accepted ordered face set and rejects non-square or volume combinations. | Source-2013 cubemaps emit right, left, back, front, up, down, and the profile-selected sphere face. | Synthetic and three exact seven-face map cubemaps pass. Six-face v7.5 evidence remains absent. | Partial |
| Version/dialect/start-frame rules determine whether a pre-7.5 sphere face exists. | The explicit Source-2013 dialect emits a sphere face before 7.5 when start frame is not `0xffff`; the broader public-contract conflict remains unresolved for denominator acceptance. | Synthetic and three configured TF2 v7.4 cubemaps produce seven correctly bounded faces. CS:S and retained target evidence remain absent. | Partial |
...
| Stable byte-addressed color formats decode channel order to native-precision canonical planes. | BGR888 decodes to tightly packed RGB/U8 with stored row order; other byte-addressed formats remain explicit non-decoded identities. | Two-pixel BGR and four exact BGR sources pass. | Partial |
