...
This leaf roadmap contains exactly 17 behavior rows. The candidate dialect inventory contains exactly 4 rows and contributes 0 denominator items until the Source format-universe denominator is Accepted, a checked-in generator emits the dialect inventory, and a denominator review accepts this roadmap. The current completion denominator is therefore the 17 behavior rows. It is Not accepted.
...
| `KeyValues::GetInt`, `GetUint64`, `GetFloat`, and `GetBool` convert an existing scalar according to its retained native type; byte strings use Source numeric-prefix conversion, a present nonnumeric string converts to zero, and only a missing key returns the caller default. | `NumericValue` exposes one non-mutating accessor authority over retained byte, integer, float, and unsigned-64 scalar values. Decimal prefix, leading whitespace/sign, exponent, no-prefix, overflow, empty, native-type, boolean, and missing/default distinctions remain separate from syntax inference. | Fixed accessor vectors compare official SDK branch behavior and Microsoft CRT `atoi` boundaries; TF2 MapRuntime integration verifies exact configured descriptive filter text without changing raw bytes. | Ready |
