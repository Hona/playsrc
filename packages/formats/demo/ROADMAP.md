...
- An ordered command list. Every command retains its zero-based ordinal, absolute source range, encoded identifier, signed tick, optional slot, profile identity, typed outer metadata, encoded payload range, exact payload bytes, and coverage classification.
...
- A well-formed accepted container has exactly one complete terminal `dem_stop` command header and no bytes after it. Truncation before that header is Malformed. Command byte 0 is not an implicit stop command.
...
| The complete 1,072-byte header preserves protocol fields, all four fixed name arrays, playback-time bits, tick count, frame count, and sign-on length without using advisory metadata as allocation authority. | No Demo implementation exists. | Header vectors varying every field, NUL position, suffix byte, float bit pattern, signed boundary, and truncation position; compare raw ranges and decoded scalars exactly with an independent little-endian extractor. | Not started |
| Sign-on length identifies an in-range command boundary while every byte in the sign-on prefix is parsed through the ordinary command rules. | No Demo implementation exists. | Fixed demos and synthetic streams with sign-on boundaries at header end, every command boundary, one byte inside each field, end of input, and beyond input; require exact boundary results and Malformed offsets. | Not started |
| Sequential command parsing preserves file order, signed ticks, demo-protocol-4 slots, source ranges, duplicate ticks, decreasing ticks, and zero-length payloads until one complete terminal stop. | No Demo implementation exists. | Generate one isolated record per inventory item and mixed streams with duplicate/decreasing ticks, both slots, zero lengths, missing stop, command 0, unknown IDs, and trailing bytes; compare command ordinals and ranges exactly. | Not started |
| `dem_signon` and `dem_packet` expose profile-sized command-info blocks, flags, camera float bits, sequence information, and one exact packet-payload boundary without decoding network messages. | No Demo implementation exists. | Demo-protocol-3 and demo-protocol-4 packet vectors: compare all 76-byte blocks, sequence integers, length fields, payload hashes, and following-command offsets against an independent structural scanner. | Not started |
| `dem_synctick` has no body and `dem_stop` terminates only after its complete profile command header. | No Demo implementation exists. | Adjacent control-record vectors and every truncation point in both command-header shapes; require exact stop range, no implicit command-0 stop, and no accepted suffix after stop. | Not started |
| `dem_consolecmd` exposes a non-negative signed length and exact command bytes without text normalization or execution. | No Demo implementation exists. | Payloads containing empty bytes, one NUL, embedded NULs, non-UTF-8 bytes, maximum configured length, negative length, overflow, and truncation; compare exact bytes and deterministic range errors. | Not started |
| `dem_usercmd` exposes its signed command sequence and exact length-delimited bytes without decoding game input. | No Demo implementation exists. | Sequence-boundary and payload-length vectors for each accepted profile; compare scalar values, byte ranges, hashes, and the next command offset exactly. | Not started |
| `dem_datatables` and `dem_stringtables` preserve their distinct identifiers and exact length-delimited network payloads without parsing tables. | No Demo implementation exists. | Empty and fixed non-empty payloads for both commands in each applicable command set; compare identifiers, lengths, bytes, and downstream networking handoff ranges exactly. | Not started |
...
| Malformed lengths, truncation, unknown commands, unknown slots, unknown flags, and all caller limits produce one deterministic classification and absolute error range under bounded streaming. | No Demo implementation exists. | Instrumented runs at each bound minus one, equal to the bound, and plus one, plus every byte truncation of isolated records; require no over-budget allocation, no panic, no resynchronization, and identical repeated errors for all chunk schedules. | Not started |
| The parsed representation is lossless and every required producer and consumer uses one Demo interface without acquiring network, replay, game, or presentation authority. | No Demo implementation or consumer integration exists. | Concatenate retained source ranges and require byte equality with each successful input; integration records prove content/application producers and networking/replay/game consumers use the current interface, while repository search finds no duplicate DEM reader, fallback, compatibility path, or stale generated inventory. | Not started |
...
- **Controlled target captures:** no retained DEM capture records exact content-build provenance for TF2, CS:S, or legacy CS:GO. `playsrc.local.json` is Missing, and no capture manifest exists in the public repository. Filesystem discovery was not attempted.
