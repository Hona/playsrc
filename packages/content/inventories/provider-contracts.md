...
| `http-object` | Immutable object descriptor with exact URL, byte length, SHA-256, range capability, representation kind, and provenance | Whole object or one exact inclusive byte range | Verified complete bytes or range bytes; Missing; integrity, range, transport, cancellation, and bound errors | Asset Service or another declared origin serves bytes; Content owns request and verification semantics |
| `map-supplement` | Map-owned logical-path index mapping each supplemental external dependency to one immutable object descriptor | Exact normalized logical path in the declared index | Verified bytes and map-supplement provenance; Missing; duplicate-path, integrity, transport, cancellation, and bound errors | Map source owns the index; Content owns provider precedence and lookup |
...
| `normal-resource` | Visit the eligible active BSP PAK first, the declared map-supplement provider second, then each selected game-content provider in plan order. For each candidate, check a valid cache entry on behalf of that provider, then its exact PAK entry, immutable object, VPK entry range, or directory path. | Return first exact bytes and provenance and do not read shadows. | Stop on a declared but failed candidate; otherwise return Missing with every eligible candidate in order. |
