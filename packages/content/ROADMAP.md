...
- Directory roots, VPK indexes and ranged-entry readers, BSP PAK indexes and entry readers, and raw-source cache entries supplied through provider adapters.
...
| Reuse raw bytes only through a cache entry keyed by provider identity, provider revision, normalized logical path, and source content hash. | No implementation exists. | Cold/warm vectors proving byte identity, origin provenance, revision invalidation, hash verification, corrupt-entry rejection, and no cross-operation reuse when provider revision is Unknown. | Not started |
