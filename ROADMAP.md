...
The current TF2 browser integration plays exact configured Valve startup media while Main Menu VGUI initializes hidden, presents exact resource-driven map loading, and consumes PSMP v3 facing, PMRQ v5/PMPO v4 frames, PMST v2 alpha, PENV v3 sky/mark/Water, PVIS v3 PVS/frustum view, PSSN v10 combat/Entity/locker, Collision v3 brush, ordered projectile event batches, and PPTX v2 graceful-stop outputs through one checked native/headed contract. Rendering additionally enforces per-face visibility, exact typed model/environment requirements, inert dynamic-light/shadow families, atomic frame/resource contracts, and aligned-capture input validation; absent lightcache/eye, Water tangent/projected-depth/underwater-overlay, shadow, explicit-command-encoder backend, and target-capture inputs remain Missing or Unsupported rather than producing fallback pixels.

The integrated package foundations add a 322-item all-class TF2 core, protocol-3 DEM framing, protocol-24 recorded-state codecs, deterministic zero-Simulation replay sessions, and typed Source entity/value/template/timer/trigger/mover/contact/save-restore behavior. The direct-map session is the sole browser gameplay authority for all nine canonical TF2 classes, exact class-script health/speed, class eye heights, Scout air dash, and RED/BLU state; only Soldier and Demoman carry implemented weapons. Recorded replay remains product-blocked on the TF2 recorded-state decoder, presentation mapping, and application upload/control path. Exact VPhysics sticky implementation plus the expanded result seam, native glyph-raster comparison, remaining opaque world mip chains, and the Tempus core/`jump_beef` zone contract remain explicit blockers; no substitute solver, fallback font, inferred course, replay resimulation, water substitute, or model-specific facing repair is permitted.
