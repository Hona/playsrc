...
export default defineConfig({
  ...
  webServer: {
    command: `bun tools/playsrc/src/cli.ts dev ${process.env.PROFILE_CTF_OBJECTIVES === "1" || process.env.PROFILE_CTF_BOTS === "1" ? "ctf_2fort" : process.env.PROFILE_COMBAT === "1" || process.env.PROFILE_UPWARD_OUTDOORS === "1" || process.env.PROFILE_ROUND_RULES === "1" || process.env.PROFILE_PICKUPS === "1" || process.env.PROFILE_MATERIAL_ANIMATION === "1" || process.env.PROFILE_SCENARIOS === "demoman" || process.env.PROFILE_SCENARIOS === "scoreboard" || process.env.PROFILE_TRACKTRAIN === "1" ? "pl_upward" : "jump_beef"}`,
