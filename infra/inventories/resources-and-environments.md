...
| `storage.asset-objects` | Immutable-object storage | Asset Store remote adapter and Asset Service origin | Provide create-if-absent immutable writes, whole and ranged origin reads for raw BSP/VPK objects, exact derived-object reads, strong read-after-write, integrity verification, retention, encryption, and deletion denial. | Accepted remote adapter, HTTP range capability, provider revision, storage identity, retention, encryption, region, and access | Unsupported |
