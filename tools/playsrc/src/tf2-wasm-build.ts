...
export async function buildTf2Wasm(config: LocalConfig, threaded = true): Promise<string> {
  ...
  const child = Bun.spawn(
    [
      cargo,
      `+${threaded ? toolchains.rust.threadedToolchain : toolchains.rust.toolchain}`,
      "build",
      "-p",
      "playsrc-tf2-wasm",
      "--target",
      ...
      "--release",
      ...(threaded ? ["--features", "threaded"] : []),
      ...(threaded ? ["-Z", "build-std=panic_abort,std"] : []),
    ],
    {
      cwd: repositoryRoot,
      env: {
        ...process.env,
        ...rustEnvironment(config.sourceCacheDir),
        ...(threaded ? {
          RUSTFLAGS: [
