...
import { compareSelectionIntervals, selectionLoadingInputs, selectionLoadingPressure, selectionPreparation, type SelectionPreparationOwner } from "../profile/selection-comparison"

const [beforeDirectory, afterDirectory, owner] = process.argv.slice(2), { sourceCacheDir } = await loadLocalConfig()
if (process.argv.length !== 5 || !["model", "visible-world", "particle"].includes(owner!) || [beforeDirectory, afterDirectory].some(value => !value || !path.resolve(value).startsWith(path.resolve(sourceCacheDir) + path.sep))) {
  throw new Error("Usage: compare-selection-transition.ts <configured-cache before directory> <configured-cache after directory> model|visible-world|particle")
}
...
const preparation = { before: selectionPreparation(before, owner as SelectionPreparationOwner), after: selectionPreparation(after, owner as SelectionPreparationOwner) }
if (!before.warm && (preparation.after.wallMilliseconds >= preparation.before.wallMilliseconds || intervals[0]!.disposition !== "proven-reduction")) throw new Error("No proven reduction of the measured team-selection blocking work")
...
const report = { status: before.warm ? "matched-warm-nonregression" : "matched-reduction", beforeDirectory, afterDirectory, intervals,
  preparation, memory, sampledPeakHeap,
