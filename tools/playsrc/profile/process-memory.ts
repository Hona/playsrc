...
export type ProcessMemory = ProfileProcess & Readonly<{ residentBytes: number | null; privateBytes: number | null }>
...
export function processMemoryCommand(platform: string, processes: readonly ProfileProcess[]): Command {
  ...
  if (platform === "win32") {
    ...
    const script = `$ErrorActionPreference='Stop'; $rows=@(foreach ($processId in @(${ids})) { try { $p=Get-Process -Id $processId -ErrorAction Stop; [pscustomobject]@{id=$p.Id;residentBytes=$p.WorkingSet64;privateBytes=$p.PrivateMemorySize64} } catch [Microsoft.PowerShell.Commands.ProcessCommandException] {} }); ConvertTo-Json -InputObject $rows -Compress`
...
export function decodeProcessMemory(platform: string, output: string, processes: readonly ProfileProcess[]): readonly ProcessMemory[] {
  const rows: Array<{ id: number; residentBytes: number; privateBytes: number | null }> = platform === "win32"
