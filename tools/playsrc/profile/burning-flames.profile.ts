...
test("Burning Flames Team Captain: real backpack equip, preview, two actors and depth", async ({ page, browser }) => {
  ...
  try {
    ...
    records.standalonePreview = await page.evaluate(() => (globalThis as any).__playsrcProfile.cosmeticPreview)
    await page.screenshot({ path: path.join(directory, "standalone-preview.png") }); desktop("standalone-preview")
