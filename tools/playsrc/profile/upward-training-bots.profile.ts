...
test("profile authored headed Upward offline-practice default roster and actual completed gameplay frames", async ({ page, context, profilePhases }, testInfo) => {
  ...
  const replay = exerciseClasses ? await startGameplayReplayJournal(page, evidenceDirectory, evidenceLabel, capturePlan.replacement ? 2 : 1) : undefined
  ...
  if (process.env.PROFILE_PARTICLE_ORIENTATION_AUDIT === "1") {
    ...
    for (const [distance, yaw] of [[3, 0], [0.75, 0], [0.25, 0], [3, Math.PI / 2]]) {
      ...
      await checkNativeWindow(nativeReader ? path.join(directory, `${label}-sprite-orientation-${results.length}.desktop.png`) : undefined)
    }
