...
test("profile headed BLU spawn-to-outdoor Upward noclip terrain", async ({ page, context }, testInfo) => {
  ...
  const frameWork = summarizeFrameTimes(state.frames.flatMap((frame) => Number.isFinite(frame.detail.total) ? [frame.detail.total!] : []))
  const fields = ["models", "projectiles", "visibility", "particleWorker", "particleDecode", "audio", "dynamicItems", "world", "viewmodel", "render", "total"]
  const timings = Object.fromEntries(fields.map((name) => [name, summarizeDistribution(state.frames.flatMap((frame) => Number.isFinite(frame.detail[name]) ? [frame.detail[name]!] : []))]))
  ...
  const report = {
    ...
    particles: {
      items: summarizeDistribution(state.frames.flatMap((frame) => Number.isFinite(frame.detail.particleItems) ? [frame.detail.particleItems!] : [])),
      batches: summarizeDistribution(state.frames.flatMap((frame) => Number.isFinite(frame.detail.particleBatches) ? [frame.detail.particleBatches!] : [])),
    },
    worker,
    gpu: state.gpu,
    allocations: {
