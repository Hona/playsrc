...
test("exact archived generation transitions, isolated tabs, warm CAS and actual compositor recovery", async ({ page, context }, testInfo) => {
  ...
  try {
    ...
    expect(visual.ticks / visual.elapsedMilliseconds * 1_000).toBeLessThan(70)
    expect(visual.generation.bundle).toBe(current.configuration.applicationBuild)
    expect(visual.generation.worker).toBe(visual.generation.bundle)
    expect(visual.generation.wasm).toBe(current.configuration.wasm.sha256)
    expect(visual.generation.staleMessages).toBe(0)
