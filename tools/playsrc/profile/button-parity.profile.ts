...
test("configured CEx borders retain authored square edges and state colors", async ({ page }, testInfo) => {
  ...
  const directory = process.env.PLAYSRC_PROFILE_RUN_DIRECTORY ?? path.join(local.sourceCacheDir, "evidence", "tf2-vgui-button-parity", `states-${Date.now()}`)
  ...
  try {
  if (await startupConsoleIdle(local.sourceCacheDir) < 2000) throw new Error("Button acceptance requires genuine native idle")
  ...
  await act(() => page.locator('[data-vgui-name="FindAGameButton"]').click())
  await act(() => page.locator('[data-vgui-name="TrainingEntry"] [data-vgui-name="ModeButton"]').click())
  ...
  await act(() => page.locator(practice).hover())
  await capture("practice-armed", practice)
  await act(() => page.keyboard.press("Escape"))
  await expect(page.locator("main")).toHaveAttribute("data-local-match-visible", "false")
  await act(() => page.locator('[data-vgui-name="FindAGameButton"]').click())
  await act(() => page.locator('[data-vgui-name="CreateServerEntry"] [data-vgui-name="ModeButton"]').click())
