...
export type MacWindowSnapshot = Readonly<{ windows: readonly Window[]; screens: readonly Bounds[] }>
...
export function admitMacWindow(snapshot: MacWindowSnapshot, browserPid: number): Readonly<{ window: Window; occluders: readonly Window[] }> {
  ...
  const occluders = snapshot.windows.slice(0, target).filter(w => w.alpha > 0 && w.bounds.Width > 0 && w.bounds.Height > 0
    && w.bounds.X < b.X + b.Width && w.bounds.X + w.bounds.Width > b.X && w.bounds.Y < b.Y + b.Height && w.bounds.Y + w.bounds.Height > b.Y)
  return { window, occluders }
}
