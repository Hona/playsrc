...
Every `profile:*` browser command owns the same machine-wide headed-browser lock, reuses an exact source-identity-matched development owner, shares verified immutable Rust/WASM and source-content snapshots across worktrees, preserves a fresh isolated Playwright context, and writes total wall-clock plus owner/browser/startup/map/scenario phase timings below configured `sourceCacheDir`. Each run and lock wait are bounded below three minutes; `--fresh` replaces the development owner without weakening genuinely cold browser-derived caches.

`bun run profile:gameui` captures Main Menu startup, one bounded five-to-ten-second combined MainMenu/Options steady-state sample, and cold standard Options opening. It replaces `sourceCacheDir/profiles/gameui/jump_beef` with `report.json`, one combined Playwright trace, phase-separated CDP timelines and V8 CPU profiles, and MainMenu/Options screenshots. The report separates RAF intervals/callback costs, long tasks, event-loop lag, DOM cardinality/mutation rates and targets, heap, CDP task/script/style/layout/paint metrics, module/function/caller/stack CPU costs, renderer-main-thread trace functions, and per-runtime frame-work reasons. It exits nonzero after writing artifacts when any frame callback reaches the five-millisecond budget.
