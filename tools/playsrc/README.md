...
`bun run profile:startup` starts the real `jump_beef` development owner, records map verification, concurrent build/cache, parallel immutable publication, inline Vite creation, listener readiness, total milliseconds, and sampled parent-process memory, writes `sourceCacheDir/profiles/startup/jump_beef/report.json`, then closes both ports. The pre-optimization command measured 189,402 ms from an empty Cargo target and 104,948 ms with warm Cargo outputs; the current repeated command measures approximately one second on the same Windows host.
