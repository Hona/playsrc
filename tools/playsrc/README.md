...
`bun run profile:gameui` starts a fresh checked `jump_beef` development owner and captures Main Menu startup plus 15 requested seconds of steady state. It replaces `sourceCacheDir/profiles/gameui/jump_beef` with `report.json`, one combined Playwright trace, separate startup/steady CDP timelines and V8 CPU profiles, and a screenshot. The report separates startup from steady RAF intervals/callback costs, long tasks, event-loop lag, DOM cardinality/mutation rates, heap, CDP task/script/style/layout/paint metrics, module/function/caller/stack CPU costs, and renderer-main-thread trace functions. It exits nonzero after writing artifacts when any frame callback reaches the five-millisecond budget.
