...
test("comparison refuses cross-machine, quality, cache, roster and viewport claims", () => {
  ...
  Object.assign(before, { roster: [{ identity: 2, class: 1, team: 2, difficulty: 1 }], capturePlan: upwardCapturePlan({}) })
  ...
  expect(compareAcceptance({ ...before, capturePlan: undefined }, { ...before, capturePlan: undefined })).toMatchObject({ comparable: false, mismatches: ["capturePlan"] })
  const requested = { ...before, capturePlan: upwardCapturePlan({ PROFILE_INTEGRATED_ACCEPTANCE: "1" }) }
  expect(compareAcceptance(requested, requested).mismatches).toContain("workerInstrumentation")
  expect(compareAcceptance(before, requested).mismatches).toContain("capturePlan")
})
