...
describe("bounded headed profile orchestration", () => {
  ...
  test("same-length generated WASM edits with restored mtime invalidate byte identity", async () => {
    ...
    await writeFile(path.join(directory, "audio_wasm.wasm"), "aaaa")
    await writeFile(path.join(directory, ".playsrc-build.json"), JSON.stringify({ schema: "playsrc-threaded-wasm-build-v2", identity: "exact", files: [{ name: "tf2_wasm_bg.wasm", bytes: 4, sha256 }, { name: "audio_wasm.wasm", bytes: 4, sha256 }] }))
  ...
  test("restores exact generated builds without sharing mutable worktree inodes", async () => {
    ...
    await writeFile(path.join(cached, "audio_wasm.wasm"), "audio")
    await writeFile(path.join(cached, ".playsrc-build.json"), JSON.stringify({ schema: "playsrc-threaded-wasm-build-v2", identity,
      files: [{ name: "tf2_wasm_bg.wasm", bytes: 5, sha256: await fileFingerprint(input) }, { name: "audio_wasm.wasm", bytes: 5, sha256: await fileFingerprint(path.join(cached, "audio_wasm.wasm")) }] }))
