...
`playsrc-source-bundle <map>` resolves the exact configured BSP/PAK/VPK closure for world, model, particle, and audio presentation. It includes MDL/VVD/DX90.VTX/ANI/PHY dependencies, selected VMT/VTF inputs, five target PCFs, eight stock WAVE resources, `game_sounds_weapons.txt`, and `soundmixers.txt`, then writes one deterministic lowercase-path `PSDB` of unchanged Source bytes.
